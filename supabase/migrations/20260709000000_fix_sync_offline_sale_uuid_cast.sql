-- Fix sync_offline_sale to handle non-UUID productIds from offline-created products.
-- The direct ::uuid cast on v_item->>'productId' fails when the product was created
-- offline and has a temp ID like "offline_1743524258054_9u9aardgm".
-- Wrap the cast in a BEGIN/EXCEPTION sub-block so invalid UUIDs set v_product_id to
-- NULL instead of crashing the entire RPC call.
